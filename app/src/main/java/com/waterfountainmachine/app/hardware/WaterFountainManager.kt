...
class WaterFountainManager private constructor(
    ...
    suspend fun initialize(): Boolean {
        return try {
            Log.d(TAG, "Initializing Water Fountain Manager...")
            ...
            val serialCommunicator = object : SerialCommunicator {
                ...
                override suspend fun connect(config: SerialConfig): Boolean {
                    Log.d(TAG, "Mock SerialCommunicator: connect() called with baud rate ${config.baudRate}")
                    connected = true
                    return true
                }
                
                override suspend fun disconnect() {
                    Log.d(TAG, "Mock SerialCommunicator: disconnect() called")
                    connected = false
                }
                ...
                override suspend fun sendData(data: ByteArray): Boolean {
                    Log.d(TAG, "Mock SerialCommunicator: sendData() called with ${data.size} bytes")
                    Log.d(TAG, "Data: ${data.joinToString(" ") { "0x%02X".format(it) }}")
                    
                    // Extract command from the data to return appropriate response
                    if (data.size >= 4) {
                        lastCommand = data[3] // Command is at position 3 in VMC protocol
                        Log.d(TAG, "Extracted command: 0x%02X".format(lastCommand))
                    }
                    return true
                }
                
                override suspend fun readData(timeoutMs: Long): ByteArray? {
                    Log.d(TAG, "Mock SerialCommunicator: readData() called for command 0x%02X".format(lastCommand ?: 0))
                    ...
                    return when (lastCommand) {
                        0x31.toByte() -> { // GET_DEVICE_ID
                            ...
                            Log.d(TAG, "Returning GET_DEVICE_ID response: ${response.joinToString(" ") { "0x%02X".format(it) }}")
                            response
                        }
                        0x41.toByte() -> { // DELIVERY_COMMAND
                            val response = createVmcResponse(0x41, byteArrayOf(0x01, 0x01)) // slot 1, quantity 1
                            Log.d(TAG, "Returning DELIVERY_COMMAND response: ${response.joinToString(" ") { "0x%02X".format(it) }}")
                            response
                        }
                        0xE1.toByte() -> { // QUERY_STATUS
                            val response = createVmcResponse(0xE1.toByte(), byteArrayOf(0x01)) // success
                            Log.d(TAG, "Returning QUERY_STATUS response: ${response.joinToString(" ") { "0x%02X".format(it) }}")
                            response
                        }
                        0xA2.toByte() -> { // REMOVE_FAULT
                            val response = createVmcResponse(0xA2.toByte(), byteArrayOf(0x01)) // success
                            Log.d(TAG, "Returning REMOVE_FAULT response: ${response.joinToString(" ") { "0x%02X".format(it) }}")
                            response
                        }
                        else -> {
                            Log.w(TAG, "Unknown command, returning generic success response")
                ...
                override suspend fun clearBuffers() {
                    Log.d(TAG, "Mock SerialCommunicator: clearBuffers() called")
                }
            }
            ...
            Log.d(TAG, "Attempting to connect to hardware with config: baud rate ${serialConfig.baudRate}")
            val connected = sdk?.connect(serialConfig) ?: false
            
            Log.d(TAG, "Hardware connection result: $connected")
            if (connected) {
                Log.d(TAG, "Connection successful, attempting to get device ID...")
                // Test connection by getting device ID
                val deviceIdResult = sdk?.getDeviceId()
                Log.d(TAG, "Device ID result: success=${deviceIdResult?.isSuccess}, exception=${deviceIdResult?.exceptionOrNull()?.message}")
                
                if (deviceIdResult?.isSuccess == true) {
                    val deviceId = deviceIdResult.getOrNull()
                    Log.i(TAG, "Connected to Water Fountain Device: $deviceId")
                    isInitialized = true
                } else {
                    val exception = deviceIdResult?.exceptionOrNull()
                    Log.e(TAG, "Failed to get device ID: ${exception?.message}")
                ...
                Log.e(TAG, "Failed to connect to hardware - SDK connection returned false")
                return false
            }
            ...
            Log.i(TAG, "Water Fountain Manager initialized successfully")
            true
            
        } catch (e: Exception) {
            Log.e(TAG, "Failed to initialize Water Fountain Manager", e)
            false
        }
    }
    ...
    suspend fun shutdown() {
        try {
            Log.d(TAG, "Shutting down Water Fountain Manager...")
            sdk?.disconnect()
            isInitialized = false
            Log.i(TAG, "Water Fountain Manager shut down successfully")
        } catch (e: Exception) {
    ...
    suspend fun dispenseWater(): WaterDispenseResult {
        if (!isReady()) {
            Log.e(TAG, "Water Fountain Manager not ready. Call initialize() first.")
        ...
        return try {
            ...
            Log.i(TAG, "Attempting to dispense water from lane $primaryLane...")
            ...
            Log.w(TAG, "Primary lane $primaryLane failed: ${result.errorMessage}")
            ...
            Log.i(TAG, "Trying fallback lanes: $fallbackLanes")
            
            for (fallbackLane in fallbackLanes) {
                Log.i(TAG, "Attempting fallback dispensing from lane $fallbackLane...")
                result = attemptDispenseFromLane(fallbackLane)
                
                if (result.success) {
                    Log.i(TAG, "Fallback successful from lane $fallbackLane")
                    laneManager.recordSuccess(fallbackLane, result.dispensingTimeMs)
                    return result
                } else {
                    Log.w(TAG, "Fallback lane $fallbackLane failed: ${result.errorMessage}")
                    laneManager.recordFailure(fallbackLane, result.errorCode, result.errorMessage)
                }
            }
            
            // All lanes failed
            Log.e(TAG, "All lanes failed to dispense water")
            return WaterDispenseResult(
                success = false,
                slot = primaryLane,
            ...
            Log.e(TAG, "Exception during water dispensing", e)
            return WaterDispenseResult(
                success = false,
                slot = 1,
        ...
        return try {
            ...
            if (result.isSuccess) {
                ...
                if (dispenseResult.success) {
                    Log.i(TAG, "Water dispensed successfully from lane $lane in ${dispenseResult.dispensingTimeMs}ms")
                } else {
                    Log.w(TAG, "Water dispensing failed from lane $lane: ${dispenseResult.errorMessage}")
                }
                dispenseResult
            } else {
                Log.e(TAG, "Water dispensing operation failed for lane $lane: ${result.exceptionOrNull()?.message}")
                WaterDispenseResult(
                    success = false,
                    slot = lane,
            ...
            Log.e(TAG, "Exception during lane $lane dispensing", e)
            WaterDispenseResult(
                success = false,
                slot = lane,
                errorMessage = "Unexpected error: ${e.message}"
    ...
    suspend fun clearFaults(): Boolean {
        if (!isReady()) {
            Log.e(TAG, "Cannot clear faults - manager not ready")
            return false
        }
        
        return try {
            Log.d(TAG, "Clearing hardware faults...")
            val result = sdk!!.clearFaults()
            val success = result.isSuccess && (result.getOrNull() == true)
            
            if (success) {
                Log.i(TAG, "Hardware faults cleared successfully")
            } else {
                Log.w(TAG, "Failed to clear faults: ${result.exceptionOrNull()?.message}")
            }
            
            success
        } catch (e: Exception) {
            Log.e(TAG, "Exception while clearing faults", e)
            false
        }
    }
